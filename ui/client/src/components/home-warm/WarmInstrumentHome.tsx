...
import {
  ...
  Vo2Card,
  WeeklyPlanCard,
  type ActivityCellState,
  type ActivityInspectionSnapshot,
  type ActivityMonthSnapshot,
  type BuildPhaseSnapshot,
  type CaloriesSnapshot,
  type CoachReadSnapshot,
  type CommitmentSnapshot,
  type EngineSnapshot,
  type QuestSideSnapshot,
  type QuestSnapshot,
...
function categoryToSport(category: TrainingCategory): WarmSportId {
...
export function buildEngineSnapshot(
  activities: Activity[],
  engine: ReturnType<typeof buildWarmHomeModel>["engine"],
): EngineSnapshot {
...
function buildQuestSnapshot(
  ...
  quest: ReturnType<typeof buildWarmHomeModel>["quest"],
): QuestSnapshot {
  const palette = ["#7c6f9e", "#a8702c"];
  const sideQuests: QuestSideSnapshot[] = challenge.quests.slice(0, 2).map((item, index) => {
...
export function buildCommitmentSnapshots(
  commitments: ReturnType<typeof buildWarmHomeModel>["commitments"],
  dataMode: "reference" | "live",
  ...
  return order.map((id) => {
    ...
    // Live: every commitment except cycling shows an "x/target" denominator;
    // cycling stays a bare count.
    ...
    // Live keeps the cards visually calm — no alarm nudge text.
    const alarm = dataMode !== "live" && target !== null && id === "calisthenics" && value < target;
    // Live: only badminton keeps a status pill ("ALL", toggled with ranked);
    // the rest drop the RECORDED/etc. label.
      ...
      : id === "foundation"
        ...
        : id === "calisthenics"
          ...
          : id === "cycling"
            ? "LOGGED"
            : "ALL";
    // Live: foundation drops its "start gently" note; others keep their metric.
...
function buildWeeklyPlanSnapshot(
  model: ReturnType<typeof buildWarmHomeModel>,
...
export function buildTrainingActivitySnapshot(
  ...
  const months: ActivityMonthSnapshot[] = Array.from({ length: 12 }, (_, index) => {
...
function buildPhaseSnapshot(
  ...
  // challenge model has no phase concept at all - fall back to the classic challenge's own
  // start/end dates so the week counter still has something real to compute from.
  ...
  return {
    ...
    milestones: (challenge.milestones ?? []).slice(0, 3).map((milestone) => {
      // Terse display prefers short_* fields; prose stays the fallback so the
      // widget reads "Free Handstand · 5s → 15s" without touching coach prose.
...
export function WarmInstrumentHome({
  ...
  const snapshots = useMemo(() => {
    ...
    const phase = buildPhaseSnapshot(challengeData, dataMode);
    const coachRead: CoachReadSnapshot = {
  ...
  const phaseLabel = `${snapshots.model.phaseName.toUpperCase()} · ${snapshots.model.blockName.toUpperCase()} · ${snapshots.phase.weekLabel}`;
  ...
  return (
      <div className={`wi-shell ${dataMode === "live" ? "is-live-data" : ""}`.trim()}>
        ...
        <InstrumentHeader
          ...
          syncHealthy={snapshots.model.syncHealthy}
          syncLabel={snapshots.model.syncLabel}
          workoutsHref="/workouts"
          currentRoute="/"
        />
        {!snapshots.model.syncHealthy ? (
